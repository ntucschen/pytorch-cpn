...
import pdb
#def get_img():
def write_to_img(key_pts, img_file):
    #np.array(result['keypoints'].reshape(17,3).astype(np.float32)
    points = np.array(key_pts).reshape(17,3).astype(np.float32)
        ...
        with torch.no_grad():
            ...
            for b in range(inputs.size(0)):
                ...
                if len(single_result) != 0:
                    ...
                    pdb.set_trace()
                    write_to_img(single_result, meta['img_path'][0]) # not sure needs 0 or i
